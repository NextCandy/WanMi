...
- 搜索与筛选：关键词、分类（下拉含全部/精品/各分类及计数）、后缀、位数（全部、1–9 位与 10 位以上，与高级筛选共用 minLength/maxLength）、包含/排除字符和域名类型，状态同步到 URL；旧链接的 `group=two/three` 解析时自动转换。
- 首页为紧凑 Hero（标题 + 三项统计）后直接进入资产列表：无独立精选区块、无分类 pills 行、无「全部资产」标题行；分类以筛选下拉呈现，「复制链接」在筛选工具栏右侧；精品域名以主列表卡片上的精品标记区分。
