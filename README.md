...
- 前台服务端搜索、后缀/位数/分类/精品筛选、排序（最新/位数/字母序）、分页、URL 可分享状态和动态联系方式；分类以常驻分类轨直接呈现并显示实时数量，不再隐藏在后台或下拉框中；不展示任何导入报价字段。
...
- 自动分类完整对齐 [dog.do](https://github.com/NextCandy/dog.do) 的数字、字母、拼音、英文与杂米判断顺序及贪心拼音拆分；自动标签与人工分类在后台分区显示，人工优先且可恢复自动，并保留 `num3…num9`、`pinyin1…pinyin4`、`alpha3/alpha4`、`mixed2/mixed3` 等子类和置信度。公共 API、前台分类轨、域名列表和详情页均使用同一套 D1 多标签分类。
