...
- 后台真实登录、会话管理（含登录地）、改密、域名 CRUD、服务端排序、列显隐、批量管理、分类管理、求购线索、结构稳定且兼容中文 Excel 的 CSV 导入/导出，以及操作日志（筛选/导出/90 天自动清理）。
- 概览仪表盘：域名与展示统计、Views 热门榜、线索漏斗、90 天到期列表；后台不再显示导入的价格、底价或报价字段。
...
- 后台为深色侧边栏 + 暖白工作区，标题、正文、表格和空状态采用一致的字号层级，统计卡片、表格行悬停与开关微交互统一缓动曲线。
